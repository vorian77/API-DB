...
router.all("/", (ctx) => {
  ctx.body = "Hello from the KidSmart API-ESP Database!"
})
router.all("/test/echo", (ctx) => {
  testEcho(ctx)
})
router.all("/test/http", async (ctx) => {
  await testHttp(ctx)
})
router.all("/esp(.*)", async (ctx) => {
  await esp(ctx)
})
router.all("/ag(.*)", async (ctx) => {
  await appGyver(ctx)
})

// app
const app = new Koa()
app.use(handleErrors).use(cors()).use(koaBody()).use(router.routes())
